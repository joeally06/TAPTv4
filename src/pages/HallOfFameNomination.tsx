...
export const HallOfFameNomination: React.FC = () => {
  ...
  useEffect(() => {
    checkNominationPeriod();
  }, []);

  const checkNominationPeriod = async () => {
    try {
      ...
      if (data?.end_date) {
        ...
        const now = new Date();
        setNominationsClosed(now > endDate);
      }
    } catch (error) {
      console.error('Error checking nomination period:', error);
    }
  };
        ...
        <section className="py-16">
          <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="bg-red-50 border-2 border-red-200 rounded-lg p-8 text-center">
              ...
              <h2 className="text-2xl font-bold text-red-800 mb-2">Nominations are Closed</h2>
              <p className="text-red-700">
                The nomination period for the Hall of Fame has ended. Please check back later for the next nomination period.
              </p>
            </div>
          </div>
        </section>
